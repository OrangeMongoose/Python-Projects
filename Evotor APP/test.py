...
def response_errors(logger, error_code, url):
    ...
    if error_code == 400:
        ...
        print_log_message(logger,
                          'При добавлении/изменении товаров в облако Эвотор возникла ошибка - '
                          'Ресурс не поддерживает использованный метод')
        err_exit(logger)

    elif error_code == 406:
        print_log_message(logger,
                          'Тип содержимого, которое возвращает ресурс не соответствует типу содержимого, '
                          'которое указанно в заголовке Accept')
        err_exit(logger)
        ...
        print_log_message(logger,
                          'Превышено максимальное количество запросов в текущем периоде. '
                          'Работу с ресурсом можно возобновить после окончания времени, '
                          'указанного в заголовке X-RateLimit-Reset.')
        err_exit(logger)
...
def main():
    ...
    print_log_message(logger, f'Количество товаров в облаке Evotor: {len(products_evotor)}')
    ...
    print_log_message(logger, f'Количество товаров Evotor после удаления: {len(products_evotor)}')
    ...
    print_log_message(logger, f'Количество товаров Evotor после добавления: {len(products_evotor)}')
    ...
    products_evotor = get_data_evotor(logger, client_data['Client_UUID'])
    print_log_message(logger, f'Количество товаров в облаке Evotor после выполнения всех операций: {len(products_evotor)}')
    ...
    print_log_message(logger, f'\nВремя выполнения: {datetime.now() - time_start}')
